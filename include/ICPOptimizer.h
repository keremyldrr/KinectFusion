...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		// TODO: [DONE] Implement the point-to-point cost function.
    ...
    T sourcePoint[3] = {T(m_sourcePoint.x()), T(m_sourcePoint.y()), T(m_sourcePoint.z())};
    T outputPoint[3];
    poseIncrement.apply(sourcePoint, outputPoint);

		residuals[0] = (outputPoint[0] - T(m_targetPoint.x())) * T(sqrt(m_weight * LAMBDA));
		residuals[1] = (outputPoint[1] - T(m_targetPoint.y())) * T(sqrt(m_weight * LAMBDA));
		residuals[2] = (outputPoint[2] - T(m_targetPoint.z())) * T(sqrt(m_weight * LAMBDA));
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		// TODO: [DONE] Implement the point-to-plane cost function.
    ...
    T normal[3] = {T(m_targetNormal.x()), T(m_targetNormal.y()), T(m_targetNormal.z())};
    T target[3] = {T(m_targetPoint.x()), T(m_targetPoint.y()), T(m_targetPoint.z())};

    residuals[0] = (normal[0] * (outputPoint[0] - target[0]) + normal[1] * (outputPoint[1] - target[1]) + normal[2] * (outputPoint[2] - target[2])) * T(sqrt(m_weight * LAMBDA));
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
				...
				// TODO: [DONE] Invalidate the match (set it to -1) if the angle between the normals is greater than 60
	...
	void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				// TODO: [DONE] Create a new point-to-point cost function and add it as constraint (i.e. residual block)
				// to the Ceres problem.
        problem.AddResidualBlock(
            PointToPointConstraint::create(sourcePoint, targetPoint, weight),
        ...
        if (m_bUsePointToPlaneConstraints) {
					...
					// TODO: [DONE] Create a new point-to-plane cost function and add it as constraint (i.e. residual block)
          // to the Ceres problem.
          problem.AddResidualBlock(
              PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, weight),
              nullptr, const_cast<double*>(poseIncrement.getData()));
